...
//----------------------------------------------------------------------------//
function DEPLACEMENT_POSSIBLE(var vitesse,i,j,x,y:integer;var trouve : boolean):boolean;
...
if Matrice_structure[i,j].Ptr_joueur<>nil//si l entite reperee est un joueur
 then                                   //alors on prend en compte ses offsets
  begin    //l entite est un joueur , on ajoute alors ses offsets a ses coordonnées
   x2 := tile_size*j +  Matrice_structure[i,j].Ptr_joueur.GETOFFSET_X;//abscisse absolue de l entite
   y2 := tile_size*i +  Matrice_structure[i,j].Ptr_joueur.GETOFFSET_Y;//ordonnee absolue de l entite
  end
 else
  begin
   x2 := tile_size*j;
...
then //le joueur regarde vers la gauche
  begin//on examine les trois cases a sa gauche
   for j := pcour_fa_joueur^.pt_visuel.GETCOLONNE-2 to pcour_fa_joueur^.pt_visuel.GETCOLONNE-1 //la colonne examinée est celle situee a sa gauche, donc au rang -1
   begin
    for i:= pcour_fa_joueur^.pt_visuel.GETLIGNE-1 to  pcour_fa_joueur^.pt_visuel.GETLIGNE+1 do
      begin//on parcourt les trois cases de la colonne de gauche qui sont autour de sa position
        if (Matrice_structure[i,j].Ptr_terrain.GETMARCHABLE = false) or (Matrice_structure[i,j].Ptr_joueur <> nil) or (Matrice_structure[i,j].Ptr_artefact <> nil)
        then//Si la case est occupee soit par un element non marchable, soit par un joueur, soit par un artefact, alors il y a potentiellement contact
         begin
           ...
           if distance >= GET_Distance(i,j,direction)//on calcul la distance entre le joueur et l entite en presence
           then   //si l entite examinée est plus proche que la precedente alors c est elle que l on retient...
            begin
              distance := GET_Distance(i,j,direction);//...et c est la distance avec elle qui devient la nouvelle reference
...
then //le joueur regarde en haut
  begin//on examine les trois cases au dessus de lui
   for i := pcour_fa_joueur^.pt_visuel.GETLIGNE-2 topcour_fa_joueur^.pt_visuel.GETLIGNE-1 do
   begin
...
begin
  possibilite := deplacement_possible(vitesse,i,j,x,y,trouve);//appel de la fonction determinant si le
...
begin
   contact := Contact_joueur(i,j,x,y);//appel de la fonction determinant si le joueur est au contact
...
//----------------------------------------------------------------------------//
function PRIORITE_COMBAT(pcour_joueur:T_ptr_VD_joueur):boolean;
